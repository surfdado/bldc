...
// Private variables
static float m2_acc_confidence_decay = 0.1;
static float m2_kp = 0.25;

void ahrs_update_mahonyref_imu(float *gyroXYZ, float *accelXYZ, float dt, ATTITUDE_INFO *att) {
	float accelNorm, recipNorm;
	float qa, qb, qc;
	...
	if (accelNorm > 0.01) {
		...
		volatile float twoKp = 2.0 * m2_kp;

		accelConfidence = calculateAccConfidence(m2_acc_confidence_decay, accelNorm, &att->accMagP);
		twoKp *= accelConfidence;
